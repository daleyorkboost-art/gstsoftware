...
export async function renderEditor(root, id) {
  ...
  const data = invoice || {
    ...
    credit_bill: false, shipping_charges: "0", shipping_gst_rate: settings.shipping_gst_rate || "0", payment_allocations: [], notes: settings.terms,
  ...
  function addItem(item = { quantity: "1", rate: "0", discount_mode: "Flat", discount_value: "0", gst_rate: "18", unit: "pcs" }) {
    ...
    node.innerHTML = `<div class="item-top"><b class="item-label">Item</b><button type="button" class="remove-item">Remove item</button></div><div class="item-grid"><div class="wide">${field("Product / item name", "product_name", item.product_name, "text", 'required maxlength="160"')}</div>${field("HSN / SAC", "hsn_sac", item.hsn_sac, "text", 'maxlength="12"')}${field("Unit", "unit", item.unit, "text", 'maxlength="20"')}${field("Quantity", "quantity", item.quantity, "number", 'required min="0.001" step="0.001"')}${field("Rate (INR)", "rate", item.rate, "number", 'required min="0" step="0.01"')}${select("Discount mode", "discount_mode", { Flat: "Flat / cash", Percent: "Percentage" }, item.discount_mode || "Flat")}${field("Discount value", "discount_value", item.discount_value ?? item.discount ?? "0", "number", 'required min="0" step="0.01"')}${select("GST rate", "gst_option", [...settings.gst_rates.map((rate) => [rate, rate + "%"]), ["custom", "Custom rate"]].reduce((all, [key, value]) => ((all[key] = value), all), {}), standard ? settings.gst_rates.find((rate) => Number(rate) === Number(item.gst_rate)) : "custom")}<label class="custom-rate" ${standard ? "hidden" : ""}>Custom GST (%)<input name="gst_rate" type="number" step="0.01" min="0" max="${esc(settings.gst_max)}" value="${esc(item.gst_rate)}"></label><div class="wide">${field("Description", "description", item.description, "text", 'maxlength="500"')}</div></div>`;
  ...
  async function calculate() {
    ...
    try {
      ...
      root.querySelector("#calculation").innerHTML = totals(result.totals, result.fields) + `<p class="badge">${esc(result.fields.payment_status)}</p>`;
  ...
  root.querySelector("#add-item").onclick = () => { addItem(); container.lastChild.querySelector("input").focus(); };
