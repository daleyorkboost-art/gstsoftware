...
For an existing v2.1 installation, back up the database, deploy this code, and import **`database/migrate-gst-inclusive.sql` once** before creating or editing invoices. Existing invoices stay Exclude GST. New invoices default to Include GST in the editor; inclusive prices have GST extracted from the entered item amount and do not receive whole-rupee rounding. Shipping remains a separate, tax-exclusive charge as shown in the form.
